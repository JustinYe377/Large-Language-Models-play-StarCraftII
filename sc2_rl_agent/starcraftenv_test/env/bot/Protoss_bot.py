...
from ...config.config import map_difficulty, map_race, map_rule_bot, map_ai_build
...
def protoss_agent_vs_build_in(transaction, lock, map_name, isReadyForNextStep, game_end_event, done_event,
    ...
    # 根据agent_type决定保存路径
    if args.agent_type in ["random", "llama2_7b", "gemini", "chatgpt", "gpt4", "glm3", "prompt1", "qwen", "glm-4",
                           "APU",
                           "claude2", "qwen_7b"]:
    ...
    temp_replay_path = f'{replay_folder}/{args.current_time}_{map}_player_race_PROTOSS_VS_BUILD_IN_AI_{difficulty}_{opposite_race}_process_{process_id}_temp.SC2Replay'
    ...
    # 游戏结束后重命名重放文件
    final_replay_path = f'{replay_folder}/{args.current_time}_{map}_player_race_PROTOSS_VS_BUILD_IN_AI_{difficulty}_{opposite_race}_process_{process_id}_{result}.SC2Replay'
...
def protoss_agent_vs_rule(transaction, lock, map_name, isReadyForNextStep, game_end_event, done_event, opposite_race,
                          opposite_bot, process_id, args):
    current_dir = os.path.dirname(os.path.abspath(__file__))

    # 根据agent_type决定保存路径
    if args.agent_type in ["random", "llama2", "gemini", "chatgpt", "gpt4", "glm3", "prompt1", "qwen"]:
        base_dir = os.path.join(current_dir, '..', '..', 'log', f'{args.agent_type}_log')
    else:
        print(f"Unknown agent_type: {args.agent_type}. Defaulting to random_log.")
        base_dir = os.path.join(current_dir, '..', '..', 'log', 'random_log')
    ...
    final_replay_path = f'{replay_folder}/{args.current_time}_{map}_player_race_PROTOSS_VS_RULE_AI_{opposite_bot}_{opposite_race}_process_{process_id}_{result}.SC2Replay'
    os.rename(temp_replay_path, final_replay_path)
    with lock:
        transaction['done'] = True
        transaction['result'] = result
...
def protoss_agent_vs_human(transaction, lock, map_name, isReadyForNextStep, game_end_event, done_event, args):
    ...
    # 根据agent_type决定保存路径
    if args.agent_type in ["random", "llama2", "gemini", "chatgpt", "gpt4", "glm3", "prompt1", "qwen"]:
        base_dir = os.path.join(current_dir, '..', '..', 'log', "vshuman", f'{args.agent_type}_log')
    else:
        print(f"Unknown agent_type: {args.agent_type}. Defaulting to random_log.")
        base_dir = os.path.join(current_dir, '..', '..', 'log', 'random_log')
